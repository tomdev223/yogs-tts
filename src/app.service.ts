...
export class AppService {
  ...
  async getTTS(
    ...
    // if (fs.existsSync(`./piper_cache/${outFile}-a.mp3`)) {
    //   const file = fs.createReadStream(
    //     join(process.cwd(), `./piper_cache/${outFile}-a.mp3`),
    //   );
    //   return new StreamableFile(file);
    // }
    // if (fs.existsSync(`./piper_cache/${outFile}-b.mp3`)) {
    //   const file = fs.createReadStream(
    //     join(process.cwd(), `./piper_cache/${outFile}-b.mp3`),
    //   );
    //   return new StreamableFile(file);
    // }
    ...
    const { stdout, stderr } = await execPromise(
      ...
      ./piper/piper \
      --model ${modelPath} \
      --output_file ./piper_cache/${outFile}.mp3 \
      --json-input`,
    );
    ...
    if (fs.existsSync(`./piper_cache/${outFile}.mp3`)) {
      ...
      if (body.filters?.lizard) {
        await execPromise(
          `ffmpeg -y -f mp3 -i ./piper_cache/${outFile}-${
            flip_flop ? 'a' : 'b'
          }.mp3 -filter_complex '[0:a] asplit [out0][out2]; [out0] asetrate=${modelRate}*0.9,aresample=${modelRate},atempo=1/0.9,aformat=channel_layouts=mono,volume=0.2 [p0]; [out2] asetrate=${modelRate}*1.1,aresample=${modelRate},atempo=1/1.1,aformat=channel_layouts=mono,volume=0.2[p2]; [p0][0][p2] amix=inputs=3' -f mp3 ./piper_cache/${outFile}-${
            flip_flop ? 'b' : 'a'
          }.mp3`,
        );
        flip_flop = !flip_flop;
      ...
      if (body.filters?.alien) {
        await execPromise(
          `ffmpeg -y -f mp3 -i ./piper_cache/${outFile}-${
            flip_flop ? 'a' : 'b'
          }.mp3 -filter_complex '[0:a] asplit [out0][out2]; [out0] asetrate=${modelRate}*0.8,aresample=${modelRate},atempo=1/0.8,aformat=channel_layouts=mono [p0]; [out2] asetrate=${modelRate}*1.2,aresample=${modelRate},atempo=1/1.2,aformat=channel_layouts=mono[p2]; [p0][0][p2] amix=inputs=3' -f mp3 ./piper_cache/${outFile}-${
            flip_flop ? 'b' : 'a'
          }.mp3`,
        );
        flip_flop = !flip_flop;
      ...
      if (body.filters?.ethereal) {
        await execPromise(
          `ffmpeg -y -f mp3 -i ./piper_cache/${outFile}-${
            flip_flop ? 'a' : 'b'
          }.mp3 -filter_complex '[0:a] asplit [out0][out2]; [out0] asetrate=${modelRate}*0.99,aresample=${modelRate},volume=0.3 [p0]; [p0][out2] amix=inputs=2' -f mp3 ./piper_cache/${outFile}-${
            flip_flop ? 'b' : 'a'
          }.mp3`,
        );
        flip_flop = !flip_flop;
      ...
      if (body.filters?.robotic) {
        await execPromise(
          `ffmpeg -y -f mp3 -i ./piper_cache/${outFile}-${
            flip_flop ? 'a' : 'b'
          }.mp3 -i  -filter_complex '[0] aresample=${modelRate} [re_1]; [re_1] apad=pad_len=512 [in_1]; [in_1] asplit=2 [in_1_1] [in_1_2]; [in_1_1] [1] afir=dry=10:wet=10 [reverb_1]; [in_1_2] [reverb_1] amix=inputs=2:weights=8 1 [mix_1]; [mix_1] asplit=2 [mix_1_1] [mix_1_2]; [mix_1_1] [2] afir=dry=1:wet=1 [reverb_2]; [mix_1_2] [reverb_2] amix=inputs=2:weights=10 1 [mix_2]; [mix_2] equalizer=f=7710:t=q:w=0.6:g=-6,equalizer=f=33:t=q:w=0.44:g=-10 [out]; [out] alimiter=level_in=1:level_out=1:limit=0.5:attack=5:release=20:level=disabled' -f mp3 ./piper_cache/${outFile}-${
            flip_flop ? 'b' : 'a'
          }.mp3`,
        );
        flip_flop = !flip_flop;
      ...
      if (body.filters?.masked) {
        await execPromise(
          `ffmpeg -y -f mp3 -i ./piper_cache/${outFile}-${
            flip_flop ? 'a' : 'b'
          }.mp3 -filter_complex 'lowpass=f=750,volume=2' -f mp3 ./piper_cache/${outFile}-${
            flip_flop ? 'b' : 'a'
          }.mp3`,
        );
        flip_flop = !flip_flop;
      ...
      if (body.filters?.radio) {
        await execPromise(
          `ffmpeg -y -f mp3 -i ./piper_cache/${outFile}-${
            flip_flop ? 'a' : 'b'
          }.mp3 -filter_complex 'highpass=f=400,volume=2' -f mp3 ./piper_cache/${outFile}-${
            flip_flop ? 'b' : 'a'
          }.mp3`,
        );
        flip_flop = !flip_flop;
